...
namespace clove {
    ...
    Expected<std::unique_ptr<GhaShader>, std::runtime_error> MetalFactory::createShaderFromFile(std::filesystem::path const &file, GhaShader::Stage shaderStage) noexcept {
        ...
        if(compilationResult.hasValue()) {
            return createShaderObject(compilationResult.getValue());
        } else {
    ...
    Expected<std::unique_ptr<GhaShader>, std::runtime_error> MetalFactory::createShaderObject(std::span<uint32_t> spirvByteCode) noexcept {
        Expected<std::string, std::runtime_error> msl{ ShaderCompiler::spirvToMSL(spirvByteCode) };
        vec3ui const workgroupSize{ ShaderCompiler::getWorkgroupSize(spirvByteCode) };
        
        if(!msl.hasValue()){
            return Unexpected{ std::move(msl.getError()) };
        }
        
        NSString *librarySource{ [NSString stringWithCString:msl.getValue().c_str() encoding:[NSString defaultCStringEncoding]] };
        ...
        return std::unique_ptr<GhaShader>{ createGhaObject<MetalShader>([library newFunctionWithName:@"main0"], workgroupSize) }; //MSL can't have main so we use main0 (generated by spirv)
    }
}
